...
Current ERA-07 work: **slice 3 DONE + exact-head GitHub-hosted CI green at `e990dff5cd6e8c7fa317d4b94840320eb6b73adb`**. The final design replaces the fragile PlayerbotFactory static diff with `tools/apply-playerbot-era-item-policy.py`, a strict post-patch transformer run after the complete wrapper + EraTalents stack in setup, update, Integration and Group Composer compile. Central provenance is the authoritative item allow/veto layer; older Playerbots/EraTalents heuristics remain only as conservative backstops. Slice 4 now moves that same policy into direct Adventure Cache rewards.
