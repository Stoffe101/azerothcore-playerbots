...
Current ERA-07 work: **slice 3 repair in progress**. The first GitHub-hosted heavy builds at `c685274a` failed during pinned-tree assembly because 0044 was applied before EraTalents' own PlayerbotFactory patch. 0044 is now regenerated for the exact post-EraTalents tree and deferred consistently in setup/update/Integration/Group Composer compile. The provenance design itself is retained. Exact head `89367cc` then failed only the fast static checks because an assertion matched comment line wrapping; that assertion is now being repaired without changing runtime behavior.
