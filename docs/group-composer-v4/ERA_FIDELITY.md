...
- GearAdvisor client guidance consumes the Group Composer server-reported realm era for baseline hit/defense cap logic when available. v0.2.0 makes the remaining limitation visible: Vanilla/TBC explicitly label detailed priority prose as a **WotLK reference** instead of silently presenting Wrath weighting as era-authentic. Full Vanilla/TBC priority research remains TODO.
