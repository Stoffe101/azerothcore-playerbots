...
- ERA-07 slice 3 is exact-head GitHub-CI green at `e990dff5`: central item provenance now reaches PlayerbotFactory-generated inventory/equipment plus AdventureStart and catch-up boundaries through a strict post-patch transformer. It fails closed before destructive/one-time state changes. Older factory heuristics may remain as extra restrictions but are not chronology evidence.
- Adventure Cache direct rewards are exact-head CI green at `e1a2e6fa`: cache readiness is checked before consuming the pending reward and generated gear/potions are central-provenance gated.
- ERA-07 automated vendor/reward-helper source `9499939` is integrated at `304aee8d` and awaits exact-head CI/runtime proof: Titan Rune vendor display/purchase/exchange, pending rewards and Gamma signets plus AI Guild stock/mail/AH/item-service helpers now require ready/allowed central provenance before protected state changes. Four custom Titan currency/signet IDs are reviewed WotLK overrides and the integrity audit covers these persistent/helper paths.
- Protocol loot injection, recipe discovery/profession crafting, ordinary loot/crafting/recipes and remaining non-PlayerbotFactory item producers still need the central item policy. Enchant **spell** chronology remains separate because item provenance cannot prove when an enchant spell became available.
- GearAdvisor v0.3 consumes the Group Composer server-reported realm era and **no longer reuses WotLK stat-priority prose in Vanilla/TBC**. Static priority output is disabled as upgrade authority. WoWSimsBridge routes the character to the matching Classic/TBC/WotLK simulation family. A sim/model must be supported and validated for that era/spec before it may claim an upgrade percentage; otherwise the UI must say UNSUPPORTED/LIMITED.
