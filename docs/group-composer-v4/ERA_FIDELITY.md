...
- ERA-07 slice 3 pushes central item provenance into PlayerbotFactory's generated equipment/inventory paths plus AdventureStart and catch-up package boundaries. The first CI attempt exposed a patch-order conflict with EraTalents; the repaired contract layers 0044 after EraTalents in every assembly path. It removes raw item-ID chronology from protected AutoGear/gem selection and fails before destructive/one-time state changes when provenance is unavailable. Exact-head CI is still required for the repair.
