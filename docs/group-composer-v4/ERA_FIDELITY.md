...
- ERA-07 slice 3 pushes central item provenance into PlayerbotFactory's generated equipment/inventory paths plus AdventureStart and catch-up package boundaries. It deliberately removes raw item-ID chronology from protected AutoGear/gem selection and fails before destructive/one-time state changes when provenance is unavailable. Exact-head CI is still required for this slice.
