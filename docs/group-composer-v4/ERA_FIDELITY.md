...
- ERA-07 slice 3 pushes central item provenance into PlayerbotFactory-generated equipment/inventory paths plus AdventureStart and catch-up boundaries. Two CI attempts proved a static PlayerbotFactory diff is too order-fragile across the wrapper + EraTalents stack, so the canonical repair uses a strict post-patch source transformer. Central provenance becomes the authoritative allow/veto layer and fail-closes before destructive/one-time state changes; older factory heuristics may remain as extra restrictions but are not chronology evidence. Exact-head CI is still required for the transformer repair.
- Vendors/rewards, loot/crafting/recipes and non-PlayerbotFactory item producers still need the central item policy. Enchant **spell** chronology is also a separate future policy because item provenance cannot prove when an enchant spell became available.
