...
- Adventure Cache direct rewards are the active slice-4 consumer. Vendors/other rewards, loot/crafting/recipes and remaining non-PlayerbotFactory item producers still need the central item policy. Enchant **spell** chronology remains separate because item provenance cannot prove when an enchant spell became available.
