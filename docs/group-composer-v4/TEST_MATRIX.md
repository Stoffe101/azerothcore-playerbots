...
- STATIC: AI Guild uses the shared `EraPolicy::ResolveCraftOutputs` + `IsAutomatedCraftSpellAllowed` path.
- STATIC: queued and autonomous AI Guild crafting validate before `ai->CastSpell`.
- STATIC: PlayerbotFactory callback registration includes the automated-craft predicate.
