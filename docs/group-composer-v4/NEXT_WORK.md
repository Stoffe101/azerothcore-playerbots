...
19b. **DONE + exact-head local-CI green at `7fe1a1be` / runtime TODO:** ERA-07 world-loot/recipe visibility + broader automated Playerbots crafting from Codex `4a24bb09`. Read-only scanning covers 13 loot-template sources; shared craft-output/recipe policy and Playerbots pre-cast guards are integrated. Loot-crafting `CREATE_ITEM_2` remains intentionally fail-closed because loaded grouped/reference outputs cannot yet be authoritatively enumerated.
19c. **NEXT ERA-07:** live audit/crafting acceptance across the same persistent Vanilla -> TBC -> WotLK progression, then decide whether historical world-loot/recipe overlays or a reviewed core runtime loot-template enumeration API are worth implementing. Do not destructively rewrite ordinary loot before evidence.
