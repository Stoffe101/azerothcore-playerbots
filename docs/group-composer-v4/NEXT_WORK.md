...
16. **CURRENT NEXT:** extend item provenance into `EquipCatchup` and starter/catch-up packages first, then vendors/rewards and the remaining automated item paths; also runtime-generate the central snapshot on the dev realm and inspect `.era audit`.
