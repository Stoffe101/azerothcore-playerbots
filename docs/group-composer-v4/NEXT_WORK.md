...
15c. **CURRENT simulator slice:** private `ac-wowsims` service builds the three pinned Classic/TBC/WotLK `wowsimcli` engines and exposes internal `/v1/sim` + `/v1/compare` contracts for baseline-vs-candidate DPS/HPS. This pass still needs exact-head CI and real Docker runtime proof. NEXT is the authoritative worldserver request builder plus GearAdvisor result/explanation transport.
