...
15. **CURRENT CLIENT QOL PASS:** package maintained Details + NoM0Re WeakAuras and the new GearAdvisor panel; exact-head GitHub CI + in-game UI/stat validation required.
16. **NEXT after that:** extend item provenance into `EquipCatchup` and starter/catch-up packages first, then vendors/rewards and the remaining automated item paths; also runtime-generate the central snapshot on the dev realm and inspect `.era audit`.
