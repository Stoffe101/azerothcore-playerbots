...
15f. **CURRENT / local CI:** engine-native preset harvester. Extract pinned upstream Average RaidSimRequests after pristine CLI build, classify route coverage, expose preset readiness, and fail closed on unclassified requests. NEXT after green: authoritative snapshot -> preset baseline request + candidate slot mutation.
