...
15e. **CURRENT / local CI:** pinned per-era model catalog. Routes are tied to exact engine commits + `proto/api.proto` blobs, uncatalogued combinations are UNSUPPORTED, and all catalogued routes remain ENGINE_PRESENT_UNVALIDATED. NEXT after green: pinned preset contract + RaidSimRequest construction.
