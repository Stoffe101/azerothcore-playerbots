...
export function createButton(parent: WoWFrame, options: ButtonOptions): UIButton {
    ...
    const outline = createOutline(frame, theme.colors.borderStrong);
    const label = createText(frame, options.text, "GameFontHighlightSmall");
    ...
    function render(): void {
        ...
        outline.setColor(selected ? accent : theme.colors.borderStrong);
    ...
    frame.SetScript("OnEnter", () => {
        if (enabled && !selected) setTextureColor(background, theme.colors.surfaceHover);
    });
