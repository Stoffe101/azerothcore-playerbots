...
-- Server commands remain authoritative. This addon only presents data returned by .guide and
-- invokes existing server/group-director actions.
...
frame:SetWidth(790); frame:SetHeight(560)
...
local function SetTab(which)
    ...
    if which == "finder" then Run(".guide finder all")
    elseif which == "compat" then Run(".guide compat all")
    elseif which == "roadmap" then Run(".guide roadmap")
    end
    AG.Refresh()
end

local tabDefs = {
    {"finder", "Finder"}, {"travel", "Travel Map"}, {"compat", "Compatibility"}, {"roadmap", "Roadmap"},
}
for i, def in ipairs(tabDefs) do
    local b = CreateFrame("Button", "AdventureGuideTab" .. i, frame, "CharacterFrameTabButtonTemplate")
    b:SetID(i); b:SetText(def[2]); b:SetWidth(120)
    if i == 1 then b:SetPoint("TOPLEFT", frame, "BOTTOMLEFT", 18, 3)
    else b:SetPoint("LEFT", tabs[tabDefs[i-1][1]], "RIGHT", -14, 0) end
    b:SetScript("OnClick", function() SetTab(def[1]) end)
    tabs[def[1]] = b
end
...
for i = 1, ROWS_PER_PAGE do
    ...
    row:SetWidth(720); row:SetHeight(27)
    row:SetPoint("TOPLEFT", 4, -25 - (i-1)*29)
    row:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight", "ADD")

    row.name = row:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    row.name:SetPoint("LEFT", 7, 0); row.name:SetWidth(285); row.name:SetJustifyH("LEFT")
    row.kind = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.kind:SetPoint("LEFT", 300, 0); row.kind:SetWidth(75); row.kind:SetJustifyH("LEFT")
    row.group = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.group:SetPoint("LEFT", 385, 0); row.group:SetWidth(75); row.group:SetJustifyH("LEFT")
    row.state = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.state:SetPoint("LEFT", 465, 0); row.state:SetWidth(130); row.state:SetJustifyH("LEFT")
    row.support = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    row.support:SetPoint("LEFT", 600, 0); row.support:SetWidth(115); row.support:SetJustifyH("LEFT")
...
detail:SetWidth(720); detail:SetHeight(64)
...
detailText:SetPoint("TOPLEFT", 8, -7); detailText:SetWidth(500); detailText:SetJustifyH("LEFT"); detailText:SetJustifyV("TOP")
...
roadmapFrame:SetAllPoints(content); roadmapFrame:Hide()
local roadmapTitle = roadmapFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
roadmapTitle:SetPoint("TOPLEFT", 12, -30); roadmapTitle:SetText("Your Progression Roadmap")
...
nowTitle:SetPoint("TOPLEFT", roadmapMeta, "BOTTOMLEFT", 0, -28); nowTitle:SetText("Do now")
local nowText = roadmapFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
nowText:SetPoint("TOPLEFT", nowTitle, "BOTTOMLEFT", 0, -8); nowText:SetWidth(680); nowText:SetJustifyH("LEFT")
local nextTitle = roadmapFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
nextTitle:SetPoint("TOPLEFT", nowText, "BOTTOMLEFT", 0, -35); nextTitle:SetText("Up next")
local nextText = roadmapFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
nextText:SetPoint("TOPLEFT", nextTitle, "BOTTOMLEFT", 0, -8); nextText:SetWidth(680); nextText:SetJustifyH("LEFT")
local roadRefresh = MakeButton(roadmapFrame, "Refresh roadmap", 130, 24)
roadRefresh:SetPoint("BOTTOMLEFT", 12, 12); roadRefresh:SetScript("OnClick", function() Run(".guide roadmap") end)

local travelFrame = CreateFrame("Frame", nil, content)
travelFrame:SetAllPoints(content); travelFrame:Hide()
local travelTitle = travelFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
travelTitle:SetPoint("TOPLEFT", 10, -25); travelTitle:SetText("Adventure Teleport Map")
local travelHelp = travelFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
travelHelp:SetPoint("TOPLEFT", travelTitle, "BOTTOMLEFT", 0, -5)
travelHelp:SetText("A compact travel board: only Guild Ready activities are clickable. Locked destinations remain unavailable server-side.")
local outlandTitle = travelFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
outlandTitle:SetPoint("TOPLEFT", 40, -85); outlandTitle:SetText("OUTLAND / TBC")
local northTitle = travelFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
northTitle:SetPoint("TOPLEFT", 400, -85); northTitle:SetText("NORTHREND / WOTLK")
local travelButtons = {}

local function BuildTravelBoard()
    for _, b in ipairs(travelButtons) do b:Hide() end
    travelButtons = {}
    local all = finderRows
    local leftY, rightY = -112, -112
    for _, entry in ipairs(all) do
        if entry.unlocked == "UNLOCKED" then
            local side = OUTLAND[entry.alias] and "left" or (NORTHREND[entry.alias] and "right" or nil)
            if side then
                local b = MakeButton(travelFrame, entry.name, 305, 22)
                if side == "left" then
                    b:SetPoint("TOPLEFT", 26, leftY); leftY = leftY - 25
                else
                    b:SetPoint("TOPLEFT", 382, rightY); rightY = rightY - 25
                end
...
function AG.UpdateDetails()
    if not selected then
        ...
        formBtn:Disable(); travelBtn:Disable()
        return
    end
    detailText:SetText(selected.name .. "\n" .. selected.note)
    if selected.unlocked == "UNLOCKED" and selected.status == "Guild Ready" then
        travelBtn:Enable(); formBtn:Enable()
    else
        travelBtn:Disable(); formBtn:Disable()
    end
...
refreshBtn:SetScript("OnClick", function()
    if mode == "compat" then Run(".guide compat all") else Run(".guide finder all") end
end)
...
function AG.Refresh()
    ...
    prev:SetShown(not isRoad and not isTravel); nextBtn:SetShown(not isRoad and not isTravel); pageText:SetShown(not isRoad and not isTravel)
    ...
    header:SetText(mode == "compat" and "Compatibility: green = Finder Ready, yellow = experimental, red = not ready" or "Supported Finder: only Guild Ready dungeons and raids")
    ...
    for i = 1, ROWS_PER_PAGE do
        ...
        if entry then
            ...
            row.support:SetTextColor(r,g,b); row.support:SetText(entry.status)
        else
...
local function ParseAG(msg)
    local payload = string.match(msg, "^%[AG%]%s*(.*)$")
    if not payload then return false end
    local p = Split(payload)

    if p[1] == "BEGIN" then
        if p[2] == "FINDER" then finderRows = {} elseif p[2] == "COMPAT" then compatRows = {} end
        return true
    end
    if p[1] == "END" then
        selected = nil; page = 1; AG.Refresh(); return true
    end
    if p[1] == "ROADMAP" then
        if p[2] == "LEVEL" then roadmap.level = p[3] or "?"
        elseif p[2] == "PROGRESSION" then roadmap.progression = p[3] or "?"
        elseif p[2] == "NOW" then roadmap.now = p[3] or ""
        elseif p[2] == "NEXT" then roadmap.next = p[3] or "" end
    ...
    if #p >= 8 then
        local entry = {
            alias=p[1], name=p[2], kind=p[3], status=p[4], level=p[5], size=p[6], unlocked=p[7], note=p[8],
        }
...
events:SetScript("OnEvent", function(self, event, arg1)
    if event == "CHAT_MSG_SYSTEM" then ParseAG(arg1 or "")
    elseif event == "PLAYER_LOGIN" then
        AdventureGuideDB = AdventureGuideDB or {}
    end
end)

-- Hide machine-readable transport lines from normal chat where this API exists in 3.3.5 clients.
...
mini:SetWidth(31); mini:SetHeight(31); mini:SetFrameStrata("MEDIUM"); mini:SetFrameLevel(8)
mini:RegisterForClicks("LeftButtonUp"); mini:RegisterForDrag("LeftButton")
local icon = mini:CreateTexture(nil, "BACKGROUND")
icon:SetWidth(20); icon:SetHeight(20); icon:SetPoint("CENTER"); icon:SetTexture("Interface\\Icons\\INV_Misc_Map_01")
local border = mini:CreateTexture(nil, "OVERLAY")
border:SetWidth(53); border:SetHeight(53); border:SetPoint("TOPLEFT"); border:SetTexture("Interface\\Minimap\\MiniMap-TrackingBorder")

local function UpdateMini()
    local angle = math.rad((AdventureGuideDB and AdventureGuideDB.pos) or 285)
    mini:ClearAllPoints(); mini:SetPoint("CENTER", Minimap, "CENTER", RADIUS*math.cos(angle), RADIUS*math.sin(angle))
end
mini:SetScript("OnDragStart", function(self)
    self:SetScript("OnUpdate", function()
        local mx,my = Minimap:GetCenter(); local scale=Minimap:GetEffectiveScale(); local px,py=GetCursorPosition(); px,py=px/scale,py/scale
        AdventureGuideDB = AdventureGuideDB or {}; AdventureGuideDB.pos=math.deg(math.atan2(py-my,px-mx)); UpdateMini()
    end)
end)
mini:SetScript("OnDragStop", function(self) self:SetScript("OnUpdate", nil) end)
mini:SetScript("OnClick", function()
    if frame:IsShown() then frame:Hide() else frame:Show(); SetTab(mode or "finder") end
end)
mini:SetScript("OnEnter", function(self)
    GameTooltip:SetOwner(self, "ANCHOR_LEFT"); GameTooltip:AddLine("Adventure Guide"); GameTooltip:AddLine("Finder, compatibility, roadmap and travel.",1,1,1); GameTooltip:Show()
end)
...
SlashCmdList["ADVENTUREGUIDE"] = function()
    frame:Show(); SetTab("finder")
end

frame:SetScript("OnShow", function() SetTab(mode or "finder"); UpdateMini() end)
UpdateMini()
SetTab("finder")
